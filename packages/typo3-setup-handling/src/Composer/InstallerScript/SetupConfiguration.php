...
class SetupConfiguration implements InstallerScriptInterface
{
    ...
    private function getSettings()
    {
        ...
        $localConfValues = [];
        if (
            file_exists($localConfFile = $configurationManager->getLocalConfigurationFileLocation())
            && false === strpos(file_get_contents($localConfFile), 'Auto generated by helhum/typo3-config-handling')
        ) {
            $localConfValues = $configurationManager->getLocalConfiguration();
        }
     ...
     *
     * @param array $localConf
     * @param array $defaultConf
     * @return array
     */
    private function cleanSettings(array $localConf, array $defaultConf): array
    {
        $cleanedLocalConf = [];
        foreach ($localConf as $key => $value) {
            if (is_array($value) && array_key_exists($key, $defaultConf)) {
                $cleanedLocalConf[$key] = $this->cleanSettings($value, $defaultConf[$key]);
            } elseif (!array_key_exists($key, $defaultConf) || $value !== $defaultConf[$key]) {
                $cleanedLocalConf[$key] = $value;
            }
        }
        return array_filter($cleanedLocalConf);
    }
    ...
    private function storeSettings(array $settings)
    {
        ...
        $fileContent .= Yaml::dump($this->cleanSettings($settings, $configurationManager->getDefaultConfiguration()), 5);
        file_put_contents(
            $settingsFile,
            $fileContent
        );
