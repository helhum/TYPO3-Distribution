...
class SettingsCommandController extends CommandController
{
    ...
    {
        if (!$rootConfigFile) {
            $rootConfigFile = getenv('TYPO3_PATH_COMPOSER_ROOT') . '/conf/'
        ...
        if ($noDev) {
            ...
            file_put_contents(
                ...
                str_replace('<?php', $localConfigurationFileContent, file_get_contents(getenv('TYPO3_PATH_COMPOSER_ROOT') . '/res/AdditionalConfiguration.php'))
            );
            $localConfigurationFileContent .= 'return [];' . chr(10);
        }
        file_put_contents(
